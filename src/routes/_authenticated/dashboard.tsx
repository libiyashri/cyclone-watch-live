...
export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({
    meta: [
      { title: "Detection Console | Cyclone Detection System" },
      {
        name: "description",
        content:
          "Upload satellite imagery and get an instant cyclone classification with confidence score, flagged regions and alert history.",
      },
      { property: "og:title", content: "Detection Console | Cyclone Detection System" },
      {
        property: "og:description",
        content: "AI cyclone classification with confidence scores and alert history.",
      },
      ...
      <header className="border-b border-border/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold">Cyclone Detection System</p>
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-4 py-8">
        <section className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="panel p-6">
            ...
            {runAnalysis.isPending && (
              <div className="mt-4 flex min-h-72 items-center justify-center rounded-xl border border-border/60 bg-secondary/20 text-sm text-muted-foreground">
                <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Scanning cloud structure…
              </div>
            )}
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-t border-border/60">
                    ...
                    <td className="py-3">
                      <span
                        className={
                          r.label === "CYCLONE" ? "text-accent" : "text-success"
                        }
                      >
